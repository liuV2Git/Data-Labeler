"""Local web application for Story 1 folder intake and document listing."""
...
from data_labeler.scanner import format_relative_paths, scan_folder
...
def _serialize_scan_result(result: ScanResult | None) -> dict[str, Any]:
    """Converts a scan result into JSON-safe data for the browser.
...
class DataLabelerHandler(BaseHTTPRequestHandler):
    """HTTP request handler for the local Story 1 web app."""

    app_state: AppState

    def do_GET(self) -> None:  # noqa: N802
        """Handles GET requests for the UI and API state."""
        ...
        if parsed.path == "/api/state":
            payload = {
                "statusMessage": self.app_state.status_message,
                **_serialize_scan_result(self.app_state.scan_result),
            }
    ...
    def do_POST(self) -> None:  # noqa: N802
        """Handles POST requests for choosing and refreshing folders."""
            ...
            if parsed.path == "/api/refresh":
                if self.app_state.selected_folder is None:
                    self._send_json(
                        {
                            "ok": False,
                            "statusMessage": "Choose a folder before refreshing.",
                            **_serialize_scan_result(self.app_state.scan_result),
                        },
...
def _build_html() -> str:
    """Builds the single-page Story 1 user interface.
  ...
  <body>
    <main class="app">
      <section class="header">
        <p class="eyebrow">Story 1</p>
        <h1>D2R Data Labeler</h1>
        <p class="subtitle">
          Select a local folder, detect support files, and review a scrollable
          list of supported documents before moving on to later workflow steps.
        </p>
      </section>
      ...
      <section class="grid">
        <div class="panel">
          ...
          <div class="list-shell" style="margin-top: 20px;">
            <div class="list-header">Schema Files</div>
        ...
        <div class="panel">
          <h2>Documents</h2>
          <p class="subtitle">
            The list is intentionally plain and scrollable so it remains usable
            for larger internal review batches.
          </p>
    ...
    <script>
      ...
      function renderState(payload) {
        ...
        renderRows(schemaListEl, payload.schemaFiles, "No schema files found");
...
def launch_app() -> None:
    ...
    The app is served from a small built-in HTTP server so the implementation
    stays in Python while avoiding fragile desktop GUI toolkit dependencies.
    """
